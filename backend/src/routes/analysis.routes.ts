...
import { store as prisma } from '../store';
...
async function runInference(imagePath: string): Promise<InferenceResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(process.env.PYTHON_BIN || (existsSync(localPython) ? localPython : 'python3'), [
      ...
      imagePath,
      uploadDir,
      process.env.YOLO_MODEL_PATH || path.resolve(__dirname, '../../weights/yolov11_flood.pt'),
    ]);
    let output = '';
    let error = '';
    child.stdout.on('data', (chunk: Buffer) => { output += chunk.toString(); });
...
async function latestSnapshot() {
  const snapshot = await prisma.floodSnapshot.findFirst({ orderBy: { recordedAt: 'desc' } });
  if (!snapshot) return null;
  const previous = await prisma.floodSnapshot.findFirst({ where: { recordedAt: { lt: snapshot.recordedAt } }, orderBy: { recordedAt: 'desc' } });
...
async function createResponsePlan(snapshotId: string, coveragePercentage: number, severity: string, settlements: number, blockedRoads: number, affectedHospitals: number) {
  ...
  return prisma.responsePlan.create({ data: { snapshotId, priority: severity, actionsJson: JSON.stringify(actions), resourcesJson: JSON.stringify(resources) } });
}
